...
public class VideoClub {
    ...
    private static double generarRenta(int dias,int cod){
        for(ITEMRenta ir : items){
            if( ir.getCodigo() == cod ){
                if( ir.getCopias() > 0 ){
                    return ir.generarRenta(dias);
                }
                else
                    return 0;
            }
    ...
    private static void ingActor(int cod, String name){
      for(ITEMRenta i: items){
          if(i.getCodigo()==cod && i instanceof DVD_RENTA){
              ((DVD_RENTA)i).ingresarActor(name);          
          }
      }
